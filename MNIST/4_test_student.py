...
ax.hist(np.array(student_norm),bins = bins, alpha=1, density=True, color = '#0571b0', range=None,  histtype='step', linewidth=2,linestyle = ':',  cumulative=False, bottom=None, align='mid',  zorder = 3, label = 'Student 4')
plt.grid(zorder = 1)
...
ax.hist(np.array(student_anom),bins = bins, alpha=1, density=True, color = '#d7191c', range=None,  histtype='step', linewidth=2,linestyle = ':',  cumulative=False, bottom=None, align='mid',  zorder = 3, label = 'Student 4')
...
ax.yaxis.set_minor_locator(MultipleLocator(5))
ax.tick_params(axis="both", which = 'both', direction="in",top=True, right=True, labelsize = fs-2)
plt.suptitle('MNIST, normal label: %s' % digit, fontsize = fs)
plt.tight_layout(rect=[0, 0, 0.98, 1])
plt.savefig('MNIST_distr.png', dpi = 600)
plt.show()
...
plt.plot(student_fpr, student_tpr, linestyle = '--', label = 'Student 4 (AUC = %s)' % student_AUC, zorder = 3)
...
plt.grid(zorder = 1, alpha = 0.8)
plt.title('MNIST, normal label: %s' % (digit), fontsize = fs)
...
plt.tight_layout()
plt.savefig('MNIST_ROC.png', dpi = 600)
plt.show()

fig, ax = plt.subplots()
plt.plot(teacher_recall, teacher_precision, label = 'Teacher (AUC = %s)' % (teacher_AUC_PR), zorder = 4)
plt.plot(student_recall, student_precision, linestyle = '--',label = 'Student 4 (AUC = %s)' % ( student_AUC_PR), zorder = 4)
plt.legend(fontsize =  fs-2, frameon=False, handlelength= 1.5, borderpad= 0.5)
plt.grid(zorder = 1, alpha = 0.8)
plt.title('MNIST, normal label: %s' % (digit), fontsize = fs)

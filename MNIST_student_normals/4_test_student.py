...
print(normals, len(test_indices))
...
plt.legend(custom_lines,labels,frameon=False, fontsize = fs, handlelength=  1.5, borderpad= 0.5)
...
ax.yaxis.set_minor_locator(MultipleLocator(5))
ax.tick_params(axis="both", which = 'both', direction="in",top=True, right=True)
plt.suptitle('MNIST Student %s, normal label: %s' % (size,digit), fontsize = fs)
plt.tight_layout()
plt.show()
...
plt.legend(fontsize = 12, frameon=False, handlelength= 1.5, borderpad= 0.5)
plt.grid(zorder = 1, alpha = 0.8)
plt.title('MNIST Student %s, normal label: %s' % (size,digit), fontsize = fs)
